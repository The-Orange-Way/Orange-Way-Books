...
const ALLOWED_REKEY_TABLES = new Set([
  ...
  'org_settings', 'attachments', 'transaction_metadata', 'account_metadata',
]);
...
const ALLOWED_CIPHERTEXT_COLUMNS: Readonly<Record<string, Set<string>>> = Object.freeze({
  transactions: new Set(['memo','encrypted_amount','encrypted_usd_value','encrypted_exchange_rate','asset','type','status','cleared_status']),
  journal_entries: new Set(['memo','ref_number','currency','encrypted_exchange_rate','status','source_type','encrypted_period_locked']),
  journal_entry_lines: new Set([
    'account_name','account_code','description',
    'encrypted_debit','encrypted_credit','encrypted_book_value',
    'encrypted_amount_native','encrypted_amount_primary','encrypted_posted_rate','encrypted_wallet_currency',
  ]),
  contacts: new Set(['name','street','city','state','zip','country','email','phone','type']),
  chart_of_accounts: new Set([
    'encrypted_name','encrypted_code','encrypted_description',
    'encrypted_account_type','encrypted_account_sub_type',
    'encrypted_is_group','encrypted_is_system','encrypted_is_archived',
  ...
  wallets: new Set(['encrypted_name','encrypted_balance','asset','account_type','connection_type','external_account_code']),
...
async function handleWrapMembers(
  ...
  if (batch.rows.length > MAX_BATCH_SIZE_WRAPS) {
    return jsonResponse({ error: `Too many wraps in one batch (max ${MAX_BATCH_SIZE_WRAPS})` }, 400, cors);
  }

  if (batch.kind === 'dek') {
    // Validate each DEK wrap row.
    const dekRows: Array<{
      org_id: string; user_id: string; wrapped_dek: string; iv: string;
      wrap_algo: string; key_version: number; is_placeholder: boolean;
    }> = [];
  ...
  const oskRows: Array<{
    user_id: string; org_id: string; key_version: number;
    wrapped_private_key: string; wrap_algo: string; iv: string;
  }> = [];
  for (let i = 0; i < batch.rows.length; i++) {
    const r = batch.rows[i] as Record<string, unknown>;
    if (typeof r.user_id !== 'string' || !UUID_RE.test(r.user_id)) {
    ...
    oskRows.push({
      user_id: r.user_id, org_id: job.org_id, key_version: job.new_osk_key_version,
      wrapped_private_key: r.wrapped_private_key, wrap_algo: r.wrap_algo, iv: r.iv,
    });
  }
...
async function handleRekeyRows(
  ...
  if (batch.rows.length > MAX_BATCH_SIZE_ROWS) {
    return jsonResponse({ error: `Too many rows in one batch (max ${MAX_BATCH_SIZE_ROWS})` }, 400, cors);
  }
  ...
  interface RekeyRowUpdate {
    table: string; row_id: string; new_dek_key_version: number;
  ...
  for (const u of updates) {
    const update: Record<string, unknown> = { dek_key_version: u.new_dek_key_version, ...u.new_ciphertext_fields };
  ...
  const newErrorLog = failedRows.length > 0
