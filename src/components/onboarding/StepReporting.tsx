  ...
  return (
    <div className="space-y-5">
        ...
        {data.secondaryCurrency === 'BTC' && (
          <div className="space-y-2 pl-1">
            ...
            <RadioGroup
              ...
              onValueChange={(v) => onChange({ ...data, secondaryBitcoinDisplay: v as BitcoinDisplay })}
              className="space-y-1.5"
            >
              {btcDisplayOptions.map((o) => (
                <div key={o.value} className="flex items-center gap-2">
                  <RadioGroupItem value={o.value} id={`btc-secondary-${o.value}`} />
                  <Label htmlFor={`btc-secondary-${o.value}`} className="font-mono text-sm cursor-pointer">
                    {o.label}
                  </Label>
                </div>
              ))}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="fmt-us" className="cursor-pointer">
                US / Standard <span className="font-mono text-xs text-muted-foreground ml-1">1,250.00</span>
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="EU" id="fmt-eu" />
              <Label htmlFor="fmt-eu" className="cursor-pointer">
                EU / International <span className="font-mono text-xs text-muted-foreground ml-1">1.250,00</span>
              </Label>
            </div>
          </RadioGroup>
        </div>
          ...
          >
            {[
              ...
              <div key={o.value} className="flex items-center gap-2">
                ...
                <Label htmlFor={`df-${o.value}`} className="cursor-pointer font-mono text-sm">{o.label}</Label>
              </div>
            ))}
          </RadioGroup>
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="tf-12" className="cursor-pointer">12-hour clock</Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="24h" id="tf-24" />
              <Label htmlFor="tf-24" className="cursor-pointer">24-hour clock</Label>
            </div>
          </RadioGroup>
        </div>
      ...
      <div className="flex gap-3">
        ...
        <Button type="button" onClick={onNext} className="flex-1">Continue</Button>
      </div>
    </div>
  );
}
