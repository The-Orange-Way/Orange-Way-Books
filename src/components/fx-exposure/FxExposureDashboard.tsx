...
export function FxExposureDashboard({ orgId, primaryCurrency }: FxExposureDashboardProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const byCurrency = new Map<string, { native: number; primary: number | null; accountIds: Set<string>; pending: number }>();
  ...
  const handleRefreshRate = useCallback(async (currency: string) => {
    setRefreshing(prev => new Set(prev).add(currency));
    try {
      await resolvePinnedRate({ source: currency, target: primaryCurrency, at: new Date().toISOString().slice(0, 10) });
      await load();
    } catch {
      // Ignore — rate may be pending
    } finally {
      setRefreshing(prev => { const s = new Set(prev); s.delete(currency); return s; });
    }
  ...
  if (exposures.length === 0) {
    return (
      <div className="text-sm text-muted-foreground py-4">
        No foreign-currency exposure detected. All accounts appear to use the primary currency ({primaryCurrency}).
      </div>
    );
  }
    ...
    <div className="space-y-4">
      {pendingRevalCount > 0 && (
        <div className="flex items-center gap-2 text-xs px-3 py-2 rounded-lg" style={{ background: '#FFF7ED', border: '1px solid #FED7AA', color: '#92400E' }}>
          ...
          {pendingRevalCount} journal entry line{pendingRevalCount !== 1 ? 's' : ''} have pending exchange rates and are excluded from totals.
        </div>
      )}
              ...
              return (
                <tr key={exp.currency} className="border-b border-border/50 hover:bg-muted/30">
                  <td className="py-2.5 pr-4">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">{exp.accountCount} account{exp.accountCount !== 1 ? 's' : ''}</span>
                    {exp.pendingCount > 0 && (
                      <span className="ml-2 text-xs px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">{exp.pendingCount} pending</span>
                    )}
                  </td>
                  <td className="text-right py-2.5 pr-4 font-mono text-xs">
                    <span className={exp.totalNative >= 0 ? 'text-green-700' : 'text-red-700'}>
                      {exp.totalNative >= 0 ? <TrendingUp className="w-3 h-3 inline mr-0.5" /> : <TrendingDown className="w-3 h-3 inline mr-0.5" />}
                      {exp.totalNative.toLocaleString(undefined, { maximumFractionDigits: 8 })}
                    </span>
                  </td>
                  <td className="text-right py-2.5 pr-4 font-mono text-xs">
                    {exp.totalPrimary != null ? formatAmount(exp.totalPrimary, primaryCurrency) : <span className="text-muted-foreground">—</span>}
                  </td>
                  <td className="text-right py-2.5 pr-4">
                    {exp.daysSinceUpdate != null ? (
                      <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${stale ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                  ...
                  <td className="text-right py-2.5">
                    ...
                    >
                      <RefreshCw className={`w-3 h-3 mr-1 ${refreshing.has(exp.currency) ? 'animate-spin' : ''}`} />
                      Refresh
                    </Button>
                  </td>
                </tr>
