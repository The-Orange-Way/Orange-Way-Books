...
export default function Payments() {
  ...
  const loadLineItemsForDialog = useCallback(
    async (paymentRequestId: string) => {
      ...
      try {
        ...
        const accountIds = Array.from(
          new Set((lineRows as any[]).map((r) => r.chart_of_accounts_id).filter(Boolean)),
        );
        ...
        if (attRows) {
          for (const a of attRows as any[]) {
            ...
            try {
              fname = a.key_version ? await decryptText(a.file_name) : a.file_name;
            } catch {
              /* keep fallback */
            }
            try {
              mime =
                a.key_version && a.mime_type
                  ? await decryptText(a.mime_type)
                  : (a.mime_type ?? null);
            } catch {
              /* keep null */
            }
        ...
        for (const row of lineRows as any[]) {
          ...
          items.push({
            ...
            amount: dec.amount,
            account_label: dec.chart_of_accounts_id
              ? (accountLabelMap[dec.chart_of_accounts_id] ?? '(account)')
              : '(uncategorized)',
  ...
  const downloadLineAttachment = useCallback(
    async (att: { storage_path: string; file_name: string; mime_type: string | null }) => {
      try {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      setRows([]);
      toast.error(
        'Could not load payment requests. Check that your vault is unlocked, then refresh.',
      );
    } finally {
      setLoading(false);
    }
  ...
  const exportPaymentsCsv = useCallback(() => {
    ...
    exportToCsv(
      `owb-payments-${format(new Date(), 'yyyy-MM-dd')}`,
  ...
  async function handleCreate(isDraft: boolean) {
    ...
    const validLineItems = formLineItems.filter(
    ...
    try {
            ...
            validLineItems.map(async (li, idx) => {
              ...
              for (const file of li.files) {
                try {
                  ...
                  if (upErr) {
                    console.warn('Line-item file upload failed:', upErr);
                    continue;
                  }
      ...
      if (forcedToPending) {
        ...
        toast.warning(
          'Payment request created. Approval threshold check skipped: exchange rate pending. Re-review once the rate resolves.',
        );
      } else {
        toast.success(
          `Payment request created${validLineItems.length > 0 ? ` with ${validLineItems.length} line items` : ''}`,
        );
      }
      resetForm();
      setNewOpen(false);
  ...
  function updateLineItem(clientId: string, patch: Partial<LineItemDraft>) {
    setFormLineItems((prev) =>
      prev.map((li) => (li.clientId === clientId ? { ...li, ...patch } : li)),
    );
  }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      const { error } = await supabase.rpc(
        'approve_payment_request' as never,
        {
          request_id: id,
  ...
  async function handleReject(id: string) {
    ...
    try {
      ...
      const { error } = await supabase.rpc(
        'reject_payment_request' as never,
        {
          request_id: id,
          new_status_ciphertext: encStatus,
  ...
  async function handleCancel(id: string) {
    try {
      ...
      const { error } = await supabase
        .from('payment_requests')
        .update({ status: encStatus, key_version: 2 })
        .eq('id', id);
      if (error) throw error;
  ...
  async function handleOnHold(id: string, reason: string) {
    if (!reason.trim()) {
      toast.error('On-hold reason is required');
      return;
    }
    setReviewSaving(true);
    try {
  ...
  async function handleBulkCancel() {
    ...
    try {
      ...
      for (const id of selected) {
        const r = rows.find((p) => p.id === id);
        if (!r || (r.status !== 'DRAFT' && r.status !== 'PENDING' && r.status !== 'ON_HOLD'))
          continue;
        const { error } = await supabase
          .from('payment_requests')
          .update({
  ...
  function handleRowClick(r: DecryptedPayment) {
    if (r.status === 'PENDING') setReviewRow(r);
      ...
      setReviewRow(r); // same dialog as PENDING; surfaces Reopen + Approve + Reject
    ...
    return (
      <div className="p-6" data-testid="payments-no-access">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          You don&apos;t have access to Payments in this organization. Ask an owner or admin to
    ...
    <div className="p-6 space-y-4" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--color-gray-500)' }}>
            ...
            {statusCounts.PENDING} pending · {statusCounts.APPROVED} approved · {statusCounts.PAID}{' '}
            paid
          </p>
        </div>
      ...
      >
        <TabsList className="h-auto flex-wrap justify-start gap-1 p-1">
          <TabsTrigger value="requests" className="gap-1.5 px-3 py-2">
            Requests
            {requestsTabBadge > 0 && (
              <span className="min-w-[1.25rem] rounded-full bg-red-500 px-1 text-center text-[10px] font-bold text-white">
                {requestsTabBadge}
              </span>
            )}
          </TabsTrigger>
          {canApprovePayments && (
            <TabsTrigger
              ...
              data-testid="payments-tab-approvals"
            >
              Approvals
              {approvalsTabBadge > 0 && (
                <span className="min-w-[1.25rem] rounded-full bg-red-500 px-1 text-center text-[10px] font-bold text-white">
            ...
            <TableRow>
              ...
              >
                <span className="flex items-center">
                  Request date <SortIcon col="created_at" />
                </span>
              </TableHead>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={formType} onValueChange={setFormType}>
                  ...
                  <SelectContent>
                    {['Invoice', 'Transfer', 'Reimbursement', 'Other'].map((t) => (
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        !formDocDate && 'text-muted-foreground',
                      )}
                    >
                  ...
                  <PopoverContent className="w-auto p-0">
                    <Calendar mode="single" selected={formDocDate} onSelect={setFormDocDate} />
                  </PopoverContent>
                </Popover>
              </div>
              <div>
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0">
                    <Calendar mode="single" selected={formDueDate} onSelect={setFormDueDate} />
                  </PopoverContent>
                </Popover>
              </div>
            </div>
            <div>
              ...
              <Input
                placeholder="BTC address, Lightning address, or bank details"
                value={formAddress}
            ...
            <div>
              ...
              <Textarea
                placeholder="Business justification for this payment..."
                value={formDesc}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>
                  Payee Email{' '}
                  <span className="text-xs text-muted-foreground">(snapshot at creation)</span>
                </Label>
                <Input
                  type="email"
            ...
            <div className="border-t pt-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={async () => {
                    await ensureAccountOptionsLoaded();
                    addLineItem();
                  }}
                >
                ...
                <>
                  <div className="space-y-2">
                    {formLineItems.map((li) => (
                      <div
                        key={li.clientId}
                        className="grid grid-cols-12 gap-2 items-start border rounded-md p-2 bg-muted/30"
                      >
                        <Input
                          ...
                          onChange={(e) =>
                            updateLineItem(li.clientId, { description: e.target.value })
                          }
                        />
                        <Input
                        ...
                        <div className="col-span-2 flex flex-col gap-1">
                          <label className="text-[10px] text-blue-700 underline-offset-2 hover:underline cursor-pointer">
                            ...
                            />
                            {li.files.length > 0
                              ? `${li.files.length} file(s) attached`
                              : 'Attach file…'}
                          </label>
                          <Button
                            type="button"
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setNewOpen(false)} disabled={formSaving}>
              Cancel
            </Button>
      ...
      >
        <DialogContent className="max-w-md">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <span className="font-mono">{reviewRow.ref_number}</span>
                </div>
                <div>
                  <span className="text-gray-500">Type:</span> {reviewRow.request_type}
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="font-mono">
                    {fmtPaymentAmount(reviewRow.amount, reviewRow.currency)}
                  </span>
                </div>
                {reviewRow.due_date && (
                  <div>
                    <span className="text-gray-500">Due:</span>{' '}
                    {format(new Date(reviewRow.due_date), 'MMM d, yyyy')}
                  </div>
                )}
                {reviewRow.vendor_ref && (
                  <div>
                    <span className="text-gray-500">Vendor Ref:</span> {reviewRow.vendor_ref}
                  </div>
                )}
              </div>
                      ...
                      {dialogLineItems[reviewRow.id]?.map((li) => (
                        <TableRow key={li.id}>
                          ...
                          <TableCell className="py-1.5 text-xs text-right font-mono">
                            {fmtPaymentAmount(li.amount, reviewRow.currency)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
              ...
              <div>
                ...
                <Textarea
                  placeholder="Why is this being rejected or placed on hold?"
                  value={rejectReason}
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              variant="destructive"
              onClick={() => reviewRow && void handleReject(reviewRow.id)}
              disabled={reviewSaving || !rejectReason.trim()}
            >
              <XCircle className="w-4 h-4 mr-1" /> Reject
            </Button>
            <Button
              onClick={() => reviewRow && void handleApprove(reviewRow.id)}
              disabled={reviewSaving}
            >
              {reviewSaving ? (
                <Loader2 className="w-4 h-4 mr-1 animate-spin" />
      ...
      >
        <DialogContent className="max-w-md">
          ...
          {processRow && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-gray-500">Ref:</span>{' '}
                  <span className="font-mono">{processRow.ref_number}</span>
                </div>
                <div>
                  <span className="text-gray-500">Status:</span> {statusBadge(processRow.status)}
                </div>
                <div>
                  <span className="text-gray-500">Payee:</span>{' '}
                ...
                <div>
                  <span className="text-gray-500">Amount:</span>{' '}
                  <span className="font-mono">
                    {fmtPaymentAmount(processRow.amount, processRow.currency)}
                  </span>
                </div>
              </div>
                    ...
                    <TableBody>
                      {dialogLineItems[processRow.id]?.map((li) => (
                        <TableRow key={li.id}>
                          ...
                          <TableCell className="py-1.5 text-xs text-right font-mono">
                            {fmtPaymentAmount(li.amount, processRow.currency)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setProcessRow(null)}>
              Close
            </Button>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <span className="text-gray-500">Amount:</span>{' '}
                  <span className="font-mono">
                    {fmtPaymentAmount(rejectionRow.amount, rejectionRow.currency)}
                  </span>
                </div>
              </div>
              <div className="p-3 bg-red-50 rounded-md text-sm">
                ...
                <span className="text-red-800">
                  {rejectionRow.rejection_reason || 'No reason provided'}
                </span>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setRejectionRow(null)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
