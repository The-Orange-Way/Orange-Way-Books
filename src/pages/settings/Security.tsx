...
export default function Security() {
  ...
  const refresh = useCallback(async () => {
    ...
    const { data: active } = await supabase
      ...
      .eq('org_id', orgId)
      .maybeSingle();
    setLastRotatedAt((active as { last_rotated_at?: string | null } | null)?.last_rotated_at ?? null);

    const { data: jobs } = await supabase
      .from('key_rotation_jobs')
      .select('id, status, trigger_type, started_at, completed_at, rollback_expires_at, rows_total, started_by, refresh_mode')
      .eq('org_id', orgId)
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <section className="rounded-lg border border-border bg-card p-5 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Refreshing your team's security issues fresh security codes and shares them
          with current team members. Anyone you've removed loses access to data they
          cached on their device.
        </p>
      ...
      <section className="rounded-lg border border-border bg-card p-5 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          One 12-word phrase that unlocks every organization you belong to. Optional —
          per-org recovery codes still work independently.
        </p>
      ...
      <section className="rounded-lg border border-border bg-card p-5 space-y-4">
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-2">
            {history.map((j) => (
              <div key={j.id} className="flex items-center justify-between gap-3 text-sm border border-border rounded-md p-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    ...
                    {j.rollback_expires_at && j.status === 'complete'
                      && new Date(j.rollback_expires_at).getTime() > now && (
                      <> · undo available until {new Date(j.rollback_expires_at).toLocaleDateString()}</>
                    )}
                  </p>
                </div>
                {j.status === 'complete'
        ...
        {rollbackable.length > 0 && (
          <p className="text-xs text-muted-foreground pt-2 border-t border-border">
            Undoing restores the previous security. Only use this if something
            is wrong after the refresh.
          </p>
        )}
      </section>

      {/* Backup format picker */}
      <Dialog open={backupOpen} onOpenChange={(o) => { if (!o && !backupWorking) setBackupOpen(false); }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Download organization backup</DialogTitle></DialogHeader>
          <div className="space-y-3 text-sm">
            <p>Choose a format for your backup. Both contain the same data.</p>
            <Select value={backupFormat} onValueChange={(v) => setBackupFormat(v as OrgBackupFormat)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <Dialog open={!!rollbackJobId} onOpenChange={(o) => { if (!o && !rollbackWorking) setRollbackJobId(null); }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Undo the last refresh?</DialogTitle></DialogHeader>
          <div className="space-y-3 text-sm">
            <p className="font-semibold">This restores your previous security.</p>
            <p className="text-muted-foreground">
              Only use this if something is wrong after the latest refresh.
              Your team will need to reload after this finishes.
            </p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setRollbackJobId(null)} disabled={rollbackWorking}>
              Cancel
            </Button>
            <Button
              variant="destructive"
              disabled={rollbackWorking}
              onClick={() => { if (rollbackJobId) void handleRollback(rollbackJobId); }}
            >
...
function friendlyTrigger(trigger: string): string {
  switch (trigger) {
    ...
    case 'post_revoke':      return 'After member removal';
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`text-[10px] font-semibold px-2 py-0.5 rounded ${v.cls}`}>{v.label}</span>;
}
