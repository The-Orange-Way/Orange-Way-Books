...
export function ChangePrimaryCurrency({
  ...
  const handleCommit = useCallback(async () => {
    ...
    if (reason.length < 40) {
      setError('Reason must be at least 40 characters.');
      return;
    }
    ...
    try {
      ...
      // Insert new era
      await supabase.from('org_primary_currency_history' as any).insert({
        org_id: orgId,
          ...
          {newCurrency && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800 space-y-2">
              <p className="font-semibold flex items-center gap-1">
                <AlertTriangle className="w-3.5 h-3.5" /> What happens
              </p>
              <ul className="list-disc list-inside space-y-1">
                <li>
                  All <em>future</em> journal entries will use <strong>{newCurrency}</strong> as
                  primary currency.
                </li>
                <li>
                ...
                <li>
                  Open A/R and A/P items (below) remain pinned in <strong>{currentPrimary}</strong>{' '}
                  until settled.
                </li>
              </ul>
            </div>
            ...
            <div className="space-y-2">
              ...
              {openItemsLoading ? (
                ...
                <p className="text-xs text-green-700">
                  No open items detected before {effectiveDate}.
                </p>
              ) : (
                <p className="text-xs text-amber-700">
                  {openItems.length} journal entries found before {effectiveDate}. These will retain{' '}
                  {currentPrimary} as their posting currency.
                </p>
              )}
            </div>
          )}
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="text-sm font-medium">
              Audit Reason{' '}
              <span className="text-muted-foreground font-normal">(minimum 40 characters)</span>
            </Label>
            <textarea
            ...
            />
            <p
              className={`text-xs ${reason.length >= 40 ? 'text-green-600' : 'text-muted-foreground'}`}
            >
      ...
      {step === 'confirm' && (
        <div className="space-y-4">
          <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 space-y-2">
            ...
            <p className="text-xs">
              You are changing the primary currency from <strong>{currentPrimary}</strong> to{' '}
              <strong>{newCurrency}</strong> effective <strong>{effectiveDate}</strong>.
            </p>
          </div>

          <div className="space-y-2">
            <Label className="text-sm font-medium">
              Type <code className="bg-muted px-1 rounded font-mono">{currentPrimary}</code> to
              confirm:
            </Label>
            <Input
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs mt-0.5">
              New transactions will use {newCurrency}. Prior entries are unchanged. Reload the app
