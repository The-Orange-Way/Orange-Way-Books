...
export default function Flash() {
  ...
  const { status, loading: statusLoading, error: statusError, updatedAt, refresh } = useFlashStatus();
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-foreground mb-1">Pay with Flash</h1>
      <p className="text-sm text-muted-foreground mb-6">
        Platform billing connection — connect the Orange Way Books Flash account once so customers can
        pay their subscription via Flash.
      </p>
      ...
      <div className="rounded-lg border border-border bg-card p-6 max-w-2xl">
        <div className="flex items-center justify-between mb-4">
          ...
          {statusLoading ? (
            <Badge variant="secondary"><Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" />Checking…</Badge>
          ) : connected ? (
        ...
        {connected && expiresAt ? (
          <div className="text-sm text-muted-foreground space-y-1">
            <p>Token expires <strong>{new Date(expiresAt).toLocaleString()}</strong>.</p>
            {updatedAt && (
              <p>Last refreshed {new Date(updatedAt).toLocaleString()}.</p>
            )}
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Click below to connect the Orange Way Books merchant Flash account. You will be redirected
              to Flash to sign in and authorize the connection, then sent back here.
            </p>
