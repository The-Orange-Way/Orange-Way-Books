...
import { Plus, Search, RefreshCw, Upload, Download, Loader2, ChevronRight, ChevronDown, Pencil, Trash2, Undo2, CalendarIcon, X, AlertTriangle, ToggleLeft, ToggleRight } from 'lucide-react';
...
import { encryptJournalEntry, decryptJournalEntry, encryptJournalEntryLine, decryptJournalEntryLine, decryptChartOfAccount, decryptOrganization, decryptOrgSettings } from '@/lib/crypto-fields';
...
import { ImportPopup, type ImportPreviewRow, type ImportResult } from '@/components/ui/import-popup';
import {
  JOURNAL_CSV_HEADERS,
  JOURNAL_SAMPLE_CSV,
...
function toCanonicalType(raw: string | null | undefined): CanonicalAcctType | null {
  if (raw === 'ASSET' | raw === 'LIABILITY' | raw === 'EQUITY' | raw === 'INCOME' | raw === 'EXPENSE') {
    return raw;
  }
...
function findPairingWarnings(
  ...
  if (typesInPlay.size > 0) {
    ...
    if (onlyPL) {
      warnings.push({
        ...
        message: 'Only income/expense accounts used. Most entries also touch a balance-sheet account (cash, payables, equity).',
      });
    }
  }
  ...
  const drTypes = new Set(dr.map(l => l.accountType).filter((t): t is CanonicalAcctType => t !== null));
  const crTypes = new Set(cr.map(l => l.accountType).filter((t): t is CanonicalAcctType => t !== null));
  ...
  if (incomeVsExpense) {
    warnings.push({
      ...
      message: 'Income and expense posted against each other. Should this flow through cash/payables/receivables instead?',
    });
  }
...
function buildJournalExportRows(
  ...
  for (const e of entryList) {
    ...
    if (elines.length === 0) {
      rows.push([e.date, e.ref_number ?? '', e.memo ?? '', e.status, '', '', '', currencyLabel, '', '']);
      continue;
    }
...
interface AccountOption { id: string; account_name: string; account_code: string | null; account_type: string; }

function AccountCombobox({ value, accounts, onChange }: {
  value: string;
        ...
        <div className="absolute z-50 w-full mt-0.5 bg-background border rounded shadow-lg max-h-48 overflow-y-auto text-xs">
          {filtered.map(a => (
            <div
              ...
              onMouseDown={() => { onChange(a.account_name); setSearch(a.account_name); setOpen(false); }}
            >
...
export default function JournalEntries() {
  ...
  const [manualRatePair, setManualRatePair] = useState<{ walletCurrency: string; primaryCurrency: string; date: string } | null>(null);
  ...
  const buildLineInsert = useCallback(async (params: {
    ...
    if (result.pending) {
      toast.warning(`Rate for ${params.wallet_currency}→${params.primary_currency} not available. Line saved as pending — resolve from the Pending Rates banner.`);
    }
  ...
  const fetchData = async () => {
    ...
    const [jeRes, sRes, acctRes, orgRes] = await Promise.all([
      supabase.from('journal_entries').select('*').eq('org_id', orgId).order('date', { ascending: false }),
    ...
    const decryptedJe = await Promise.all(je.map(async (e: any) => {
    ...
    if (je.length > 0) {
      ...
      for (const key of Object.keys(grouped)) {
        grouped[key] = await Promise.all(grouped[key].map(async (l: any) => {
    ...
    if (acctRes.data) {
      const decryptedAccts = await Promise.all(
        (acctRes.data as any[]).map(async (a) => {
          ...
          return { id: a.id, account_name: fields.account_name, account_code: fields.account_code, account_type: fields.account_type };
        })
      );
    ...
    if (sRes.data) {
      ...
      if (decSettings.primary_currency) setPrimaryCurrency(decSettings.primary_currency.toUpperCase());
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      else if (sortKey === 'memo') { av = (a.memo | '').toLowerCase(); bv = (b.memo | '').toLowerCase(); }
      else { av = entryTotals[a.id] | 0; bv = entryTotals[b.id] | 0; }
  ...
  const fmtAmount = useCallback((amount: number, currency: string) => {
    if (currency === 'BTC' | currency === 'SATS') return formatCrypto(currency === 'SATS' ? amount / 1e8 : amount, btcDisplay);
  ...
  const exportJournalCsv = useCallback(() => {
    ...
    exportToCsv(`owb-journal-entries-${format(new Date(), 'yyyy-MM-dd')}`, [...(JOURNAL_CSV_HEADERS as readonly string[])], rows);
    toast.success(`Exported ${sorted.length} journal entr${sorted.length === 1 ? 'y' : 'ies'} to CSV (${rows.length} rows).`);
  ...
  const statusBadge = (status: string) => {
    ...
    return <Badge className="text-[10px]" style={{ background: s.bg, color: s.color, border: 'none' }}>{status}</Badge>;
  };
  ...
  const openEdit = (entry: JournalEntry) => {
    ...
    setFLines(entryLines.length >= 2 ? entryLines.map(l => ({
    ...
    try {
      if (editingEntry) {
        ...
        const encLines = await Promise.all(
          fLines.filter(l => l.debit | l.credit).map(async (l) => {
  ...
  const handleReverse = async (id: string) => {
    if (!confirm('Reverse this journal entry? A new reversing entry will be posted in the current period.')) return;
    try {
  ...
  const handleBulkPost = async () => {
    ...
    try {
      ...
      for (const id of selected) {
        ...
        if (error) continue;
        writeAuditLog({
          orgId: orgId!, action: 'POST', entityType: 'journal_entry', entityId: id,
          summary: 'Bulk posted journal entry', encrypt: encryptText,
        });
        moved += 1;
      }
  ...
  const handleBulkReverse = async () => {
    if (selected.size === 0) return;
    if (!confirm(`Reverse ${selected.size} selected entries? A reversing JE will be posted in the current period for each.`)) return;
    setBulkActing(true);
      ...
      for (const id of selected) {
        ...
        try {
          await reverseJournalEntry({
            ...
            reason: 'Bulk reverse', encryptText, decryptText,
          });
          moved += 1;
        } catch (err: any) {
  ...
  const handleBulkDelete = async () => {
    ...
    const blockers = Array.from(selected)
      ...
      .filter((e): e is typeof entries[number] => !!e && (e.status !== 'DRAFT' | e.period_locked));
    if (blockers.length > 0) {
      alert(`${blockers.length} entries cannot be deleted (only DRAFT + unlocked rows). Reverse posted entries instead.`);
      return;
    }
  ...
  const toggleSelectAll = (pageIds: string[]) => setSelected(prev => {
  ...
  return (
    <div className="p-6 space-y-4" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canWriteJE && (
            ...
            >
              <Plus className="w-4 h-4 mr-1" />Add Journal Entry
            </Button>
          )}
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        ...
        {selected.size > 0 && (
          <div className="flex flex-wrap items-center gap-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm mb-3">
            ...
            <Button variant="ghost" size="sm" disabled={bulkActing} onClick={() => setSelected(new Set())}>
              Clear
            </Button>
          </div>
        )}
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer select-none" onClick={() => toggleSort('memo')}>
                  <span className="flex items-center">Purpose<SortIcon col="memo" /></span>
                </TableHead>
            ...
            <TableBody>
              {sorted.map(entry => {
                ...
                const canReverse = entry.status === 'POSTED' && !entry.period_locked && !entry.source_type;

                return (
                  <>{/* entry row */}
                    <TableRow
                      key={entry.id}
                      className={cn('hover:bg-[#fafafa] dark:hover:bg-muted/40', canEdit && 'cursor-pointer')}
                    ...
                    >
                      ...
                      <TableCell className="font-mono text-xs">
                        <span className="inline-flex items-center gap-1">
                          <button onClick={e => { e.stopPropagation(); toggleExpand(entry.id); }} className="p-0.5">
                      ...
                      <TableCell>
                        {entry.ref_number && (
                          <Badge variant="outline" className="text-[10px] font-mono">{entry.ref_number}</Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <span className="text-xs max-w-[200px] truncate">{entry.memo ? (entry.memo.length > 40 ? entry.memo.slice(0, 40) + '...' : entry.memo) : '—'}</span>
                          {statusBadge(entry.status)}
                          {entry.period_locked && <Badge className="text-[9px]" style={{ background: '#FEF3C7', color: '#D97706', border: 'none' }}>Locked Period</Badge>}
                        </div>
                      </TableCell>
                      <TableCell className="text-right font-mono text-sm">
                        {fmtAmount(total, entry.currency)}
                        {entry.currency !== primaryCurrency && entry.exchange_rate && entry.exchange_rate !== 1 && (
                          <span className="block text-[0.72rem]" style={{ color: 'var(--color-gray-400)' }}>
                            1 {entry.currency} = {Number(entry.exchange_rate).toFixed(6)} {primaryCurrency}
                          </span>
                        )}
                      </TableCell>
                      <TableCell>
                        <Badge className="text-[10px]" style={{
                          background: entry.currency === 'BTC' | entry.currency === 'SATS' ? 'var(--color-brand-orange-light)' : '#EFF6FF',
                          color: entry.currency === 'BTC' | entry.currency === 'SATS' ? 'var(--color-brand-orange)' : '#2563EB',
                          border: 'none',
                        }}>{entry.currency}</Badge>
                      </TableCell>
                      <TableCell onClick={e => e.stopPropagation()}>
                        <div className="flex gap-1">
                          {canEdit && canWriteJE && <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEdit(entry)} data-testid="je-edit-button"><Pencil className="w-3.5 h-3.5" /></Button>}
                          {canEdit && canDeleteJE && <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => handleDelete(entry.id)} data-testid="je-delete-button"><Trash2 className="w-3.5 h-3.5" /></Button>}
                          {canReverse && <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleReverse(entry.id)} title="Reverse"><Undo2 className="w-3.5 h-3.5" /></Button>}
                        </div>
                      </TableCell>
                    </TableRow>
            ...
            return (
              ...
              >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1.5">
                        <span className="text-xs truncate">
                          {entry.memo ? (entry.memo.length > 50 ? entry.memo.slice(0, 50) + '…' : entry.memo) : '—'}
                        </span>
                        {statusBadge(entry.status)}
                        {entry.period_locked && (
                          <Badge className="text-[9px]" style={{ background: '#FEF3C7', color: '#D97706', border: 'none' }}>
                            Locked
                          </Badge>
                        )}
                      </div>
                ...
                <div className="mt-2 flex justify-end gap-0.5" onClick={e => e.stopPropagation()}>
                  {canEdit && canWriteJE && (
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEdit(entry)} data-testid="je-edit-button">
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                  )}
                  {canEdit && canDeleteJE && (
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => handleDelete(entry.id)} data-testid="je-delete-button">
                      <Trash2 className="w-3.5 h-3.5" />
                    </Button>
                  )}
                  {canReverse && (
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleReverse(entry.id)} title="Reverse">
        ...
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              >
                {simpleMode ? <ToggleRight className="w-4 h-4 text-primary" /> : <ToggleLeft className="w-4 h-4" />}
          ...
          <div className="space-y-4">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-full justify-start text-left">
                      <CalendarIcon className="w-4 h-4 mr-2" />{format(fDate, 'MM/dd/yyyy')}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={fDate} onSelect={d => d && setFDate(d)} className="p-3 pointer-events-auto" />
                  </PopoverContent>
                </Popover>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Ref #</Label>
                <Input value={fRefNum} onChange={e => setFRefNum(e.target.value)} placeholder="JE26-000001" />
              </div>
              <div>
                <Label>Memo / Purpose</Label>
                <Input value={fMemo} onChange={e => setFMemo(e.target.value)} placeholder="Description" />
              </div>
            </div>

            {/* Pinned-rate chip — shown when wallet currency ≠ primary */}
            {fCurrency && primaryCurrency && fCurrency.toUpperCase() !== primaryCurrency.toUpperCase() && (
              <PinnedRateChip
            ...
            <div>
                  ...
                  <TableBody>
                    {fLines.map((line, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className="p-1">
                          {fLines.length > 2 && (
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setFLines(fLines.filter((_, j) => j !== i))}>
                              <X className="w-3 h-3" />
                            </Button>
                          )}
                        </TableCell>
              ...
              <Button variant="ghost" size="sm" className="text-xs mt-2" onClick={() => setFLines([...fLines, { account_code: '', account_name: '', debit: '', credit: '', description: '' }])}>
                + Add Line
              </Button>
            </div>
            ...
            {pairingWarnings.length > 0 && (
              <div className="space-y-1.5">
                {pairingWarnings.map((w) => (
                  <div key={w.code} className="flex items-start gap-2 px-3 py-2 rounded-lg border border-yellow-200 bg-yellow-50 dark:bg-yellow-950/30 dark:border-yellow-800">
                    <AlertTriangle className="w-4 h-4 text-yellow-600 shrink-0 mt-0.5" />
                    <span className="text-xs text-yellow-800 dark:text-yellow-300">{w.message}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Totals */}
            <div className="flex items-center justify-between px-2 py-2 rounded-lg" style={{ background: 'var(--color-gray-50)' }}>
              <div className="flex gap-6 text-sm">
                <span>Total Debits: <strong className="font-mono">{fmtAmount(totalDebits, fCurrency)}</strong></span>
                <span>Total Credits: <strong className="font-mono">{fmtAmount(totalCredits, fCurrency)}</strong></span>
              </div>
              <div>
                {isBalanced ? (
                  <span className="text-green-600 text-sm font-medium">✓ Balanced</span>
                ) : totalDebits > 0 | totalCredits > 0 ? (
                  <span className="text-red-600 text-sm font-medium">✗ Out of balance by {fmtAmount(Math.abs(totalDebits - totalCredits), fCurrency)}</span>
                ) : null}
              </div>
            </div>
          </div>
      ...
      {manualRatePair && (
        <ManualRateDialog
          open={manualRateOpen}
          onClose={() => { setManualRateOpen(false); setManualRatePair(null); }}
...
function PinnedRateChip({
  walletCurrency, primaryCurrency, date,
}: { walletCurrency: string; primaryCurrency: string; date: string }) {
  const { rate, loading, stale, pending, asOf, provider } = useExchangeRateChip(walletCurrency, primaryCurrency, date);
