...
const HAVE_LOCAL_CREDS = (() => {
  try { return !!readSupaCreds(); } catch { return false; }
})();
test.skip(!HAVE_LOCAL_CREDS, '/tmp/owb-pw/owb-dev-supabase.json not present — cross-user spec runs only on dev laptops');
...
test.describe.serial('RLS cross-user — user A cannot see user B', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (cr.status === 200 | cr.status === 201) {
      ...
      const q = await adminFetch(supa.url, supa.secret, `/auth/v1/admin/users?email=${encodeURIComponent(USER_B_EMAIL)}`, 'GET');
    ...
    const memQ = await adminFetch(supa.url, supa.secret, `/rest/v1/org_members?user_id=eq.${userBId}&select=org_id`, 'GET');
  ...
  test.afterAll(async () => {
    if (userBOrgId) {
      await adminFetch(supa.url, supa.secret, `/rest/v1/organizations?id=eq.${userBOrgId}`, 'DELETE').catch(() => undefined);
    }
  ...
  test('user A cannot read user B via any cross-tenant query path', async ({ page }) => {
    ...
    await signIn(page);
    await page.goto('https://dev.books.orangeway.app/app', { waitUntil: 'networkidle' });
    ...
    const callerFetch = async (path: string): Promise<{ status: number; body: string }> => {
      return page.evaluate(async ({ url, key, token, p }) => {
    ...
    expect(sawB, `user A must NOT see user B's org ${userBOrgId} in broad SELECT — actually got ${orgRows.length} rows`).toBe(false);
    ...
    expect(memRows.length, `user A must get 0 rows when querying org_members for B's user_id`).toBe(0);
    ...
    for (const table of ['chart_of_accounts', 'wallets', 'journal_entries', 'contacts', 'org_settings', 'invoices']) {
