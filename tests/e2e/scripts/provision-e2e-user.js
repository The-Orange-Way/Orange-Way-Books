...
(async () => {
  ...
  const ref = (() => {
    try { return new URL(owb.url).hostname.split('.')[0]; } catch { return ''; }
  })();
  if (!ALLOWED_PROJECT_REFS.has(ref)) {
    ...
    console.error('  Only OWB DEV is allowed. Edit ALLOWED_PROJECT_REFS in this file to add a ref.');
    process.exit(2);
  }
  ...
  if (cr.status === 422 || cr.status === 200 || cr.status === 201) {
    ...
    console.error('  user create failed:', cr.body); process.exit(1);
  }
  ...
  const onboardVisible = await page.locator('text=Set Up Orange Way Books').isVisible({ timeout: 3000 }).catch(() => false);
  const unlockVisible = await page.locator('text="Unlock your encrypted vault"').isVisible({ timeout: 1500 }).catch(() => false);
  ...
  if (unlockVisible) {
    ...
    const stillLocked = await page.locator('button:has-text("Unlock Vault")').isVisible().catch(() => false);
    if (stillLocked) { console.error('  unlock rejected — vault pw drift'); process.exit(2); }
    ...
    for (let i = 0; i < 6; i++) {
      ...
      try { await btn.click({ force: true, timeout: 1500 }); } catch {}
    }
  ...
  await page.waitForTimeout(2000);
  const url = page.url();
  if (url.includes('/login')) { console.error('  ended up at /login — session lost'); process.exit(3); }
  console.log('→ final URL:', url);

  fs.writeFileSync('/tmp/owb-pw/e2e-creds.json', JSON.stringify({
    email: EMAIL, password: PASSWORD, vault_password: VAULT_PW,
  }, null, 2));
